...
async function main() {
  ...
  // Supprimer les anciens clients démo
  await prisma.client.deleteMany({
    where: {
      companyId: demoCompany.id,
      email: { in: demoClients.map(c => c.email) }
    }
  });
  ...
  // Supprimer les anciens produits démo
  await prisma.produit.deleteMany({
    where: {
      companyId: demoCompany.id,
      nom: { in: demoProduits.map(p => p.nom) }
    }
  });
  ...
  // Supprimer les anciens employés démo
  await prisma.employe.deleteMany({
    where: {
      companyId: demoCompany.id,
      matricule: { in: demoEmployes.map(e => e.matricule) }
    }
  });
