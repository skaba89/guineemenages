...
import { Facture, Client, Company, LigneFacture } from '@prisma/client';
...
const formatMontant = (centimes: number, currency: string = 'GNF'): string => {
    ...
    try {
      ...
      const doc = new PDFDocument({
        ...
        info: {
          ...
          Author: facture.company.nom,
          Subject: `Facture ${facture.numero} du ${formatDate(facture.dateEmission)}`,
        },
      });
      ...
      doc.fillColor(COLORS.primary).fontSize(24).font('Helvetica-Bold').text(facture.company.nom, 50, 50);
      ...
      if (facture.company.ninea) {
        doc.text(`NINEA: ${facture.company.ninea}`, 50, companyY);
        companyY += 12;
      }
      ...
      doc.fillColor(COLORS.muted).fontSize(10).font('Helvetica').text(`Date: ${formatDate(facture.dateEmission)}`, 400, 105, { align: 'right' });
      
      if (facture.dateEcheance) {
        doc.text(`Échéance: ${formatDate(facture.dateEcheance)}`, 400, 120, { align: 'right' });
      }
      ...
      doc.text(facture.statut.replace('_', ' '), 400, facture.dateEcheance ? 140 : 125, { align: 'right' });
      ...
      for (const ligne of facture.lignes) {
        ...
        // Désignation
        let designation = ligne.description;
        ...
        // Prix unitaire
        doc.text(formatMontant(ligne.prixUnitaire, facture.company.devise), 365, y + 5, { width: 85, align: 'right' });

        // Total ligne
        doc.text(formatMontant(ligne.montantTTC, facture.company.devise), 450, y + 5, { width: 90, align: 'right' });
      ...
      doc.text(formatMontant(facture.montantHT, facture.company.devise), 450, y, { width: 90, align: 'right' });
      y += 20;

      // TVA
      if (facture.montantTVA > 0) {
        doc.text('TVA', 350, y);
        doc.text(formatMontant(facture.montantTVA, facture.company.devise), 450, y, { width: 90, align: 'right' });
        y += 20;
      }
      ...
      doc.text(formatMontant(facture.montantTTC, facture.company.devise), 450, y, { width: 90, align: 'right' });
...
export const generateBulletinPaiePdf = async (
  bulletin: {
    employe: { nom: string; prenom: string; poste?: string | null; numeroCNSS?: string | null };
    mois: number;
    annee: number;
    salaireBase: number;
    brutTotal: number;
  ...
  company: { nom: string; adresse?: string | null; ninea?: string | null }
): Promise<Buffer> => {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
      ...
      // En-tête
      doc.fillColor(COLORS.primary).fontSize(20).font('Helvetica-Bold').text(company.nom, 50, 50);
      ...
      if (bulletin.employe.numeroCNSS) {
        doc.text(`CNSS: ${bulletin.employe.numeroCNSS}`, 50, y);
      }
      ...
      doc.text(formatMontant(bulletin.salaireBase), 300, y, { width: 80, align: 'right' });
      doc.text(formatMontant(bulletin.salaireBase), 430, y, { width: 100, align: 'right' });
      y += 18;
      ...
      doc.text(formatMontant(bulletin.brutTotal), 430, y, { width: 100, align: 'right' });
      y += 30;
      ...
      doc.text(formatMontant(bulletin.brutTotal), 300, y, { width: 80, align: 'right' });
      ...
      doc.text(formatMontant(bulletin.brutTotal - bulletin.cnssEmploye), 300, y, { width: 80, align: 'right' });
      doc.text('-', 380, y, { width: 50, align: 'center' });
      doc.text(`-${formatMontant(bulletin.ipr)}`, 430, y, { width: 100, align: 'right' });
      y += 18;

      if (bulletin.autresRetenues > 0) {
        doc.text('Autres retenues', 55, y);
        doc.text(`-${formatMontant(bulletin.autresRetenues)}`, 430, y, { width: 100, align: 'right' });
        y += 18;
      }
      ...
      doc.fillColor(COLORS.primary).fontSize(14).text(formatMontant(bulletin.netAPayer), 430, y, { width: 100, align: 'right' });
      ...
      doc.text(formatMontant(bulletin.cnssEmployeur), 430, y, { width: 100, align: 'right' });
